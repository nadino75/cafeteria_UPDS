# Usuarios CRUD Module Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the Usuarios CRUD module (list, create, edit, deactivate) matching the Productos pattern.

**Architecture:** Single-file component `UsuariosView.vue` with Teleport modal. Route nested under `/dashboard/`. Hardcoded ROLES array (same as RegisterView.vue). Password toggle on edit.

**Tech Stack:** Vue 3 (Composition API, `<script setup>`), Tailwind v4 (café roast tokens), axios client with JWT interceptor.

---

### Task 1: Register `/dashboard/usuarios` route

**Files:**
- Modify: `resources/js/router/index.js:48-53`

- [ ] **Add the child route**

Insert after the `productos` route inside the `/dashboard` children array:
...
git commit -m "feat: add /dashboard/usuarios route"
```

---

### Task 2: Create UsuariosView.vue

**Files:**
- Create: `resources/js/views/usuarios/UsuariosView.vue`

- [ ] **Create the component**

Full content of `UsuariosView.vue`:
...
```bash
git add resources/js/views/usuarios/UsuariosView.vue
git commit -m "feat: add UsuariosView with list, modal, and deactivate"
```
...
Run: `npm run build`
Expected: All modules transformed, build succeeds (output includes `UsuariosView-*.js` asset)

- [ ] **Commit any fixes** if build fails (unlikely)
