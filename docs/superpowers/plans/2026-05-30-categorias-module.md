# Categorías CRUD Module Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use subagent-driven-development (recommended) or executing-plans to implement this plan.

**Goal:** Build the Categorías CRUD module (list, create, edit, delete).

**Architecture:** Single-file component `CategoriasView.vue` with Teleport modal. Same pattern as ProductosView/UsuariosView.

**Tech Stack:** Vue 3 (Composition API, `<script setup>`), Tailwind v4, axios + JWT.

---

### Task 1: Register route

**Files:** Modify `resources/js/router/index.js`

- [ ] Insert after the `usuarios` route:
...
- [ ] Commit: `git add resources/js/router/index.js && git commit -m "feat: add /dashboard/categorias route"`

---

### Task 2: Create CategoriasView.vue

**Files:** Create `resources/js/views/categorias/CategoriasView.vue`

Same structure as UsuariosView/ProductosView, but with:
- **Table columns:** Nombre, Descripción, Aplica a, Acciones
- **Filter:** text input for nombre
- **Modal fields:** nombre (text), descripcion (textarea), aplica_a (select: Producto, Menú, Ambos)
- **Delete:** `DELETE /categorias/{id}` with FK guard (handle 409 conflict with alert)
- **Rename `desactivar` to `eliminar`** since delete is a hard-delete (with FK guard)
...
const APLICA_A = { producto: 'Producto', menu: 'Menú', ambos: 'Ambos' }
...
- [ ] Commit: `git add resources/js/views/categorias/CategoriasView.vue && git commit -m "feat: add CategoriasView with list, modal, and delete"`

---

### Task 3: Verify build

- [ ] `npm run build` — expect 102 modules, `CategoriasView-*.js` asset built
