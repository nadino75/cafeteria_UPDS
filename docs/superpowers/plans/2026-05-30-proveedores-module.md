# Proveedores CRUD Module Implementation Plan

> **For agentic workers:** Use subagent-driven-development.

**Goal:** Build Proveedores CRUD (list, create, edit, deactivate). Same pattern as Productos/Categorías.

**Tech Stack:** Vue 3 + Tailwind v4 + axios JWT.
...
Commit: `git add resources/js/router/index.js && git commit -m "feat: add /dashboard/proveedores route"`

---

### Task 2: Create ProveedoresView.vue

**Files:** Create `resources/js/views/proveedores/ProveedoresView.vue`
...
| activo | boolean | soft-delete via destroy |

**Table columns:** Nombre Empresa, Contacto, Email, Teléfono, Activo, Acciones

**Modal fields:** nombre_empresa, contacto_nombre, email, telefono, direccion

**Actions:** Create (POST), Edit (PUT), Deactivate (DELETE — soft-delete → `desactivar`)
...
Commit: `git add resources/js/views/proveedores/ProveedoresView.vue && git commit -m "feat: add ProveedoresView with list, modal, and deactivate"`

---

### Task 3: Verify build

- [ ] `npm run build` — expect 103 modules, `ProveedoresView-*.js` built
